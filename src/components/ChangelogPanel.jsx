...
const CHANGELOG = [
  {
    date: '2026-04-21',
    title: 'Bug fix：迴圈返回不再要求 `_g` 尾碼',
    items: [
      '修正 Excel 上傳 validator 過度要求「迴圈返回（若未通過則返回…若通過則序列流向…）」的任務加 `_g` 後綴',
      '業務語意上，迴圈返回是「具迴圈行為的一般任務」，不是獨立的閘道元件，L4 編號維持一般任務格式（範例 `5-4-11-4`）',
      'validator 現在只對真正的 fork/merge 閘道檢查 `_g` 尾碼：`條件分支至`、`並行分支至`、`條件合併來自多個分支`、`並行合併來自`',
      'Parser 端保留迴圈條件的路由邏輯（流程圖渲染與連線不變），僅鬆綁編號規則',
    ],
  },
  {
    date: '2026-04-22',
