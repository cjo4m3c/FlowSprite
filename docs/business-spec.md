...
每章末尾「對應實作」小節列出檔案路徑，改規則時順著檔案改即可。
...
**對應實作**：`src/components/Dashboard/`（L3 列表）、`src/components/FlowEditor/`（L3 編輯）、`src/components/HelpPanel.jsx`（HIERARCHY array）
...
- **格式驗證 regex 的單一來源**：`src/utils/taskDefs.js`
  - `L3_NUMBER_PATTERN = /^\d+-\d+-\d+$/`
  - `L4_NUMBER_PATTERN = /^\d+-\d+-\d+(_g\d*)?$/`（含 `_g` 變體）
  - `L4_START_PATTERN = /^\d+-\d+-\d+-0$/`
  - `L4_END_PATTERN = /^\d+-\d+-\d+-99$/`
  - `L4_GATEWAY_PATTERN = /^\d+-\d+-\d+-\d+_g\d*$/`
- **Excel 匯入 parser**：`src/utils/excelImport.js`（寬鬆，吃新舊格式）
- **顯示計算**：`src/utils/taskDefs.js` `computeDisplayLabels`（**唯一 source of truth**）
- **資料遷移**：`src/utils/storage.js` `loadFlows.normalizeNumber`
...
**對應實作**：`src/diagram/constants.js`（顏色 / 尺寸常數）、`src/components/DiagramRenderer/shapes.jsx`（5 種形狀繪製）、`src/components/HelpPanel.jsx` ELEMENTS array
...
**對應實作**：`src/components/FlowEditor/RightDrawer.jsx` ConnectionSection（連線型態 UI）、`src/utils/taskDefs.js` `applyConnectionType`（連線型態變更時的副作用）、`src/components/HelpPanel.jsx` CONNECTIONS array
...
**對應實作**：`src/diagram/layout/`（11 個 phase 檔案，每檔處理一種情境）、`HANDOVER.md` §2.5（dr/dc 表 / 8 個 phase / corridor slot 詳細規則）
...
**對應實作**：`src/utils/taskDefs.js` `computeDisplayLabels`（唯一決定顯示 / 不顯示的地方）
...
**對應實作**：`src/model/validation.js`（共用驗證函式）、`src/components/FlowEditor/validateFlow.js`（薄包裝）、`src/utils/excelImport.js`（匯入時 call）、`src/components/HelpPanel.jsx` VALIDATION array
...
**對應實作**：`src/components/DiagramRenderer/`（圖上互動）、`src/components/FlowEditor/`（drawer + ContextMenu）、`src/hooks/useDragReorder.js`、`src/components/HelpPanel.jsx` EDITABLE_ACTIONS array
...
**對應實作**：`src/utils/violations.js`（連線違規偵測）、`src/model/validation.js`（儲存檢核）、`src/components/HelpPanel.jsx` FORBIDDEN_RULES array
...
**對應實作**：`src/components/DiagramRenderer/index.jsx` `handleExport`（PNG via html-to-image）、`src/utils/drawioExport.js`、`src/utils/excelExport.js`、`src/components/HelpPanel.jsx` EXPORTS array
...
**對應實作**：`src/utils/excelImport.js`（parser regex）、`src/utils/excelExport.js`（只產新格式）
...
| ③ | 編輯器（drawer flow tab + roles tab + Wizard） | `src/components/FlowEditor/` + `RightDrawer.jsx` + `Wizard.jsx` |
| ④ | 流程圖下方 Excel 表 | `src/components/FlowTable.jsx` |
| ⑤ | 下載：Excel | `src/utils/excelExport.js` |
| ⑥ | 下載：drawio | `src/utils/drawioExport.js` |
| ⑦ | 下載：PNG | `src/components/DiagramRenderer/index.jsx` `handleExport`（html-to-image） |
