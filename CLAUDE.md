...
- `.claude/business-rules.md` — 5 條核心業務規則 + 閘道分類 + 工程慣例
...
- **不是獨立閘道**（一般任務，不用 `_g`）：merge target（`X 合併來自...`）/ `迴圈返回至 X`。詳見 `.claude/business-rules.md` §2
...
3. **CLAUDE.md / 外部規則檔同步**：改 regex = §3、改 push 流程 = §1 / §2、改檔案結構 = §6 拆檔表、改業務規則 = `.claude/business-rules.md`、孤兒清理 = `.claude/orphans.md`
...
5 條核心業務規則（端點不混用 / 避免重疊 / target 順序 / 編號顯示分層 / 兩層儲存檢核）+ 閘道分類 + 工程慣例詳見 **`.claude/business-rules.md`**。

CLAUDE.md 只列規則編號摘要：

| # | 名稱 | 一句話 |
|---|---|---|
| 1 | 端點不混用 | 任一 port 不可同時 IN+OUT。違反比線交叉嚴重。 |
| 2 | 避免視覺重疊 | 線不可跨任務矩形。優先改端點 > 改路徑 |
| 3 | target 順序排 slot | 多條連線並存時 slot 內外順序按 target 欄左→右 |
| 4 | 編號顯示分層 | 流程圖只顯示正式 L3/L4；`-0` / `-99` / `_g*` 不顯示 |
| 5 | 兩層儲存檢核 | Blocking（結構不合法擋儲存）+ Warning（跳 modal 由使用者決定） |

完整描述、判定範圍、歷史教訓 → `.claude/business-rules.md`。
